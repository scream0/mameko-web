...
export default function AnalyticsChart() {
  ...
  const getTransactionAmount = (tx: any) => {
    return Number(tx.price || tx.total || tx.total_price || tx.total_amount || tx.gross_amount || tx.amount || 0);
  };
  ...
  const processChartData = useCallback((transactions: any, currentFrame: any) => {
  ...
  const processYearlySummary = useCallback((transactions: any) => {
    const yearsMap = {};
  ...
  useEffect(() => {
    const fetchTransactionData = async () => {
      try {
        ...
        transactions = transactions.filter((tx: any) => paidStatuses.has(String(tx.status || "").toLowerCase()));
